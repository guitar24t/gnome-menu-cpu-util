...
const { classifyCore } = Me.imports.lib.stats.hybrid;
...
const CoreBars = GObject.registerClass(
...
const CoreBarsItem = GObject.registerClass(
...
var CpuUtilMenu = class CpuUtilMenu {
    constructor(menu, caps, settings) {
        ...
        this._throttleBanner.visible = false;
        ...
        this._perCoreSep = new PopupMenu.PopupSeparatorMenuItem('Per-core');
        menu.addMenuItem(this._perCoreSep);
    ...
    update(sample) {
        ...
        if (this._caps.hasRapl) {
            this._powerRow.visible = showRapl;
            this._powerCoresRow.visible = showRapl;
            this._powerUncoreRow.visible = showRapl;
            if (!sample.raplAccessible) {
                this._powerRow.setValue('N/A — run setup/install-rapl-access.sh');
                this._powerCoresRow.visible = false;
                this._powerUncoreRow.visible = false;
            } else if (sample.rapl) {
                const pkg = sample.rapl.domains.find(d => /package/i.test(d.name));
                this._powerRow.setValue(fmtWatts(pkg ? pkg.watts : null));
                const cores = pkg && pkg.sub.find(s => /core/i.test(s.name));
                const uncore = pkg && pkg.sub.find(s => /uncore|gpu/i.test(s.name));
                this._powerCoresRow.visible = showRapl && !!cores;
                this._powerUncoreRow.visible = showRapl && !!uncore;
                if (cores)
        ...
        this._perCoreSep.visible = showPerCore;
        this._coreBars.visible = showPerCore;
    ...
    _showThrottleBanner(count) {
        ...
        this._throttleBanner.visible = true;
