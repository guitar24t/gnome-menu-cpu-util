...
6. **The legacy build's `prefs.js` runs under GTK 4, not GTK 3.** GNOME Shell 40+ extension prefs always load with GTK 4 — the extensions service has already required it before our file runs, so `imports.gi.versions.Gtk = '3.0'` is a no-op and `imports.gi.Gtk` is GTK 4. That means: use `frame.set_child(grid)` not `frame.add(grid)`, `box.append(child)` not `box.add(child)`, drop `show_all()` (visible by default), and never call `box.get_children()` (use `get_first_child()`/`get_next_sibling()` or keep your own list). `Gtk.Frame`, `Gtk.Grid.attach`, `Gtk.Switch`, `Gtk.SpinButton`, `Gtk.ComboBoxText`, `Gtk.CheckButton` all still exist with the same constructors.
7. **No `TextDecoder` in legacy GJS.** GS 40 ships GJS 1.68; the global `TextDecoder` wasn't added until GJS 1.70 (GS 41). The legacy build decodes bytes with `imports.byteArray.toString(bytes)` in `legacy/lib/intel.js#readText` — works across the whole 40-44 range. Don't switch the legacy file to `TextDecoder`; the modern build uses it because GS 45 ships GJS 1.76.
